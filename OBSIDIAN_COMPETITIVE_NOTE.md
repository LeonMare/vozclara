...
| ✅ DONE (code) | W3-4 | MCP Obsidian output — shipped as an additive `format: 'standard' \| 'obsidian'` param on the existing `vozclara_generate_pack` tool (NOT a separate tool — convention-correct per CLAUDE.md "composite tools > granular"). `format="obsidian"` returns YAML-frontmatter Markdown the agent writes straight to the vault. In `worker/src/mcp/agent.ts` (`wrapObsidian`). **Needs deploy** (`cd worker && npx wrangler deploy`) — then promote `/api/mcp/pro` as "Claude-Code-callable from inside your Obsidian workflow." Founder-Deal framing (cheaper than a DIY weekend) still TODO in copy. |
