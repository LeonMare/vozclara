...
| ✅ DONE | W2 | German `/notes` reading-note ON the Ivanov video itself — `/notes/julian-ivanov-claude-video-pipeline` (slug, R403PGBArDY). Intercepts the exact keyword cluster (zweites Gehirn, Claude Code Obsidian, KI-Automatisierung); engages the DIY-pipeline argument editorially and lands the export-to-Obsidian reconciliation. Shipped in `src/data/creatorNotes.ts` + sitemap. **Still TODO:** an EN sibling for r/ObsidianMD + r/ClaudeAI, and verify Ivanov's exact @handle (currently `julianivanov` w/ TODO marker). |
...
| ✅ DEPLOYED | W3-4 | MCP Obsidian output — shipped as an additive `format: 'standard' \| 'obsidian'` param on the existing `vozclara_generate_pack` tool (NOT a separate tool — convention-correct per CLAUDE.md "composite tools > granular"). `format="obsidian"` returns YAML-frontmatter Markdown the agent writes straight to the vault. In `worker/src/mcp/agent.ts` (`wrapObsidian`). **Live in production** — worker version `6ae707f6-731f-4fa4-a2d8-36619f270583`. Verified end-to-end via production MCP `tools/call`: returns valid frontmatter + `[!info]` footer + AI-Act watermark; cross-lingual works through this path (Arabic source → English pack). Still TODO in copy: promote `/api/mcp/pro` as "Claude-Code-callable from inside your Obsidian workflow" + Founder-Deal framing (cheaper than a DIY weekend). |
