...
type McpEnv = {
  ...
  /**
   * Anthropic + AI-Gateway credentials surfaced so `callLLM` can
   * route Pro Plus MCP traffic to Sonnet 4.5 (worker/src/llm-router.ts).
   * Anonymous Phase-1 calls leave `this.props?.tier` undefined →
   * callLLM defaults to 'free' → these keys never get touched.
   */
  ...
  private registerGeneratePack(): void {
    ...
    this.server.registerTool(
      ...
      async ({ url, language, depth }) => {
        ...
        // 3. Build the prompt, route through callLLM. Anonymous
        //    Phase-1 calls have no this.props → tier defaults to
        //    'free' → Llama. Authenticated /api/mcp/pro callers with
        //    this.props.tier === 'pro_plus' get Sonnet 4.5 via the
        //    AI Gateway (worker/src/llm-router.ts).
        const prompt = buildPrompt(transcriptText, detectedLang, language, depth);
        let aiResponse: string;
        let aiProvider: 'workers-ai' | 'anthropic' = 'workers-ai';
        let aiModel = 'llama-3.3-70b-instruct';
        try {
          const result = await callLLM(
            {
              ...
              maxTokens:
                depth === 'deep' ? 2_500 : depth === 'standard' ? 1_500 : 600,
              temperature: 0.4,
              // System prompt is a deterministic function of
              // (language, depth, detectedLang). Cache it on the
              // Anthropic path — once a Pro Plus user has produced a
              // pack at depth=X / language=Y, every subsequent pack
              // with the same params hits the cache. Below 1024 tokens
              // the cache silently no-ops, which is safe.
        ...
        // Attribution string matches the actual provider — Pro Plus
        ...
        return {
          content: [
            ...
            {
              type: 'text',
              text: `\n\n— Pack URL (open for interactive view, flashcards, quiz): ${packUrl}\n— AI-generated by VozClara using ${modelLabel}. Verify before relying on this output.`,
            },
          ],
          structuredContent: {
            videoId,
            ...
            packUrl,
            provenance: {
              // Dynamic — reflects the actual model that ran.
              // EU AI Act Art. 50(2) requires the synthetic-content
              // marker AND honest provenance.
              model: aiModel,
              provider: aiProvider,
