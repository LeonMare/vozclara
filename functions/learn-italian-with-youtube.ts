/**
 * Cloudflare Pages Function — /learn-italian-with-youtube
...
const META = {
  ogTitle: 'Learn Italian with the YouTube videos you already watch · VozClara',
  ogDescription:
    'Paste an Italian YouTube link — Rai News, Easy Italian, Marco Montemagno, Tech Princess — and get vocabulary at your CEFR level, a summary in your language, timestamped quotes, and an Anki deck.',
  twitterDescription:
    'Italian YouTube video → CEFR-tuned vocabulary + native-language summary + Anki deck. Close-cousin language for ES/PT, third-language pick for Anglo / German.',
} as const;
...
export async function onRequestGet(context: FunctionContext): Promise<Response> {
  ...
  const canonical = `${origin}/learn-italian-with-youtube`;
  ...
  const rewritten = html
    ...
    .replace(/<meta property="og:image:alt" content="[^"]*"\s*\/>/, `<meta property="og:image:alt" content="${escAttr('Five Anki flashcards on cordovan leather, Italian vocabulary pairs.')}" />`)
    ...
    .replace(/<meta name="twitter:image:alt" content="[^"]*"\s*\/>/, `<meta name="twitter:image:alt" content="${escAttr('Anki flashcards with Italian vocabulary pairs.')}" />`);
